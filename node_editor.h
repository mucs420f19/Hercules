...
void node_editor_add(struct node_editor *editor, const char *name, struct nk_rect bounds,
        struct node_data data, int in_count, int out_count, node_functions ftable, bool infinite_inputs = false, int gapped_inputs = 0, struct nk_color gapped_color = nk_rgb(0, 0, 0))
{
    struct node *node;
        ...
        nk_layout_space_begin(ctx, NK_STATIC, total_space.h, nodeedit->node_count);
        {
            ...
            for (n = 0; n < nodeedit->link_count; ++n) {
                ...
                nk_stroke_curve(canvas, l0.x, l0.y, l0.x + 50.0f, l0.y,
                        l1.x - 50.0f, l1.y, l1.x, l1.y, 3.0f, nk_rgb(0, 0, 0));
            }
            ...
            while (it) {
                ...
                if (nk_group_begin(ctx, it->name, NK_WINDOW_MOVABLE|NK_WINDOW_BORDER|NK_WINDOW_TITLE|NK_WINDOW_SCALABLE))
                {
                    ...
                    nk_layout_row_push(ctx, 250);
                        {
                            nk_label(ctx, "___________________________", NK_TEXT_CENTERED);
                        }
                ...
                }
                {
                    ...
                    for (n = 0; n < it->output_count; ++n) {
                        ...
                        nk_fill_circle(canvas, circle, nk_rgb(0, 0, 0));
                    ...
                    for (n = 0; n < inputs; ++n) {
                        ...
                        circle.y = node->bounds.y + space * (float)(n+1);
                        circle.w = 15; circle.h = 15;
                        nk_fill_circle(canvas, circle, n < it->input_gapped ? it->gapped_color :  nk_rgb(0, 0, 0));
            ...
            if (drawLink)
                nk_stroke_curve(canvas, link_l0.x, link_l0.y, link_l0.x + 50.0f, link_l0.y,
                        link_l1.x - 50.0f, link_l1.y, link_l1.x, link_l1.y, 3.0f, nk_rgb(0, 0, 0));
